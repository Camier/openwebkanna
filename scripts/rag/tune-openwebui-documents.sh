...
DEFAULT_RAG_EMBEDDING_MODEL="pritamdeka/S-PubMedBert-MS-MARCO"
TUNE_EMBEDDING_MODEL="${TUNE_EMBEDDING_MODEL:-${RAG_EMBEDDING_MODEL:-$DEFAULT_RAG_EMBEDDING_MODEL}}"
...
show_help() {
    cat <<'EOF'
Usage: ./scripts/rag/tune-openwebui-documents.sh [OPTIONS]
...
Tuning knobs (env overrides):
  ...
  TUNE_EMBEDDING_MODEL      Defaults to RAG_EMBEDDING_MODEL or pritamdeka/S-PubMedBert-MS-MARCO
...
ensure_api_key() {
    ...
    print_info "  OPENWEBUI_API_KEY='<token>' ./scripts/rag/tune-openwebui-documents.sh --apply"
    exit 1
}

...
3. backfill page-level `figure_records` from local migration artifacts
...
from services.rag.materialize_evidence import make_page_id, make_point_id


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--manifest", type=Path, help="Materialization manifest JSON used to resolve figure JSONL.")
    parser.add_argument("--figures-jsonl", type=Path, help="Materialized figure JSONL. Overrides --manifest for stage 2.")
    parser.add_argument(
        "--extractions-root",
        type=Path,
        default=None,
    ...
    parser.add_argument(
        "--smiles-repo-root",
        type=Path,
        default=Path("/LAB/@thesis/SMILES"),
        help="Canonical SMILES tooling root used by payload backfill.",
    )
    ...
    parser.add_argument("--payload-scroll-batch-size", type=int, default=256)
    parser.add_argument("--payload-preview-limit", type=int, default=5)
    ...
    parser.add_argument("--skip-payload-backfill", action="store_true")
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Run payload backfill in preview mode and keep stage 3 read-only.",
    )
...
def main() -> int:
    ...
    text_dense_dim = resolve_dense_dimension(client, args.source_collection, args.source_dense_vector)
    ...
    if not args.skip_figures:
        ...
        if figure_stage_input is None and args.manifest is None:
            generated_figures_jsonl, materialization_report = materialize_figures_from_extractions(
                repo_root=repo_root,
        ...
        if figure_stage_input is None and args.manifest is None:
            raise SystemExit("Figure indexing requires --manifest, --figures-jsonl, or a usable extraction root.")
        figure_args = [
            "--qdrant-url",
            args.qdrant_url,
            "--collection-name",
    ...
    if not args.skip_payload_backfill:
        payload_args = [
            "--scroll-batch-size",
            str(args.payload_scroll_batch_size),
            "--preview-limit",
            str(args.payload_preview_limit),
        ]
        if not args.dry_run:
            payload_args.append("--apply")
        stage_reports["payload_backfill"] = run_stage(
            repo_root / "scripts/rag/migrate_qdrant_smiles_payloads.py",
            payload_args,
            qdrant_api_key=args.qdrant_api_key,
            qdrant_url=args.qdrant_url,
            env_overrides={
                "MULTIMODAL_RETRIEVAL_API_EXTRACTIONS_ROOT": str(extractions_root),
                "MULTIMODAL_RETRIEVAL_API_SMILES_REPO_ROOT": str(args.smiles_repo_root.resolve()),
            },
        )
    ...
    summary = {
        ...
        "generated_figures_jsonl": str(generated_figures_jsonl) if generated_figures_jsonl is not None else None,
...
def materialize_figures_from_extractions(*, repo_root: Path, extractions_root: Path) -> tuple[Path, dict[str, Any]]:
    ...
    with output_path.open("w", encoding="utf-8") as handle:
        for normalized_path in sorted(extractions_root.glob("*/normalized.json")):
            ...
            figures = normalized.get("figures") if isinstance(normalized.get("figures"), list) else []
            block_index = normalized.get("block_index") if isinstance(normalized.get("block_index"), list) else []
            blocks_by_id = {
            ...
            for figure_index, figure in enumerate(figures, start=1):
                ...
                figure_id = (
                    ...
                    if isinstance(figure.get("figure_id"), str) and figure.get("figure_id")
                ...
                if image_uri is None:
                    ...
                    continue
                bbox = normalize_bbox(figure.get("bbox") if isinstance(figure.get("bbox"), list) else block.get("bbox"))
                payload = {
                    ...
                    "ocr_text": (
                        ...
                        else html_to_text(block.get("html") if isinstance(block.get("html"), str) else None)
                    ),
                    "image_uri": image_uri,
                    "bbox": bbox,
                    "figure_kind": (
                        block.get("block_type") if isinstance(block.get("block_type"), str) and block.get("block_type") else "other"
                    ),
                    "has_smiles": False,
                    "linked_molecule_ids": [],
                }
...
def resolve_image_uri(repo_root: Path, extraction_dir: Path, storage_uri: str | None) -> str | None:
    if not storage_uri:
        return None
...
def build_collection_summary(client: QdrantClient, collection_name: str) -> dict[str, Any]:
    return {
...
def count_points(
    ...
    *,
    ...
    if object_type is not None:
        conditions.append(models.FieldCondition(key="object_type", match=models.MatchValue(value=object_type)))
    if extra_conditions:
        conditions.extend(extra_conditions)
    count_filter = models.Filter(must=conditions) if conditions else None
    response = client.count(collection_name=collection_name, count_filter=count_filter, exact=True)

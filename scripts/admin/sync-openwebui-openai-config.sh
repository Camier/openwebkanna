...
# - Creates a DB backup via ./scripts/admin/backup-openwebui-db.sh
#
...
main() {
    ...
    if ! host_python="$(resolve_host_python)"; then
        print_error "python3 or python is required"
        exit 1
    fi
    ...
    if ! init_compose_cmd >/dev/null; then
        print_error "Docker Compose command unavailable"
        exit 1
    fi

    if ! compose_service_running "${OPENWEBUI_SERVICE:-openwebui}"; then
        print_error "OpenWebUI service '${OPENWEBUI_SERVICE:-openwebui}' is not running"
        print_error "Start it first: docker compose up -d ${OPENWEBUI_SERVICE:-openwebui}"
        exit 1
    fi

    local openwebui_service="${OPENWEBUI_SERVICE:-openwebui}"
    local openwebui_container_name="${OPENWEBUI_CONTAINER_NAME:-$openwebui_service}"
    ...
    if [ -x "./scripts/admin/backup-openwebui-db.sh" ]; then
        ...
        print_warning "./scripts/admin/backup-openwebui-db.sh not found or not executable; skipping DB backup"
    fi

    print_step "Normalizing .env (OPENAI_API_KEYS)"
    OPENAI_KEY="$openai_key" ENV_PATH="$env_file" "$host_python" - <<'PY'
import os
import re

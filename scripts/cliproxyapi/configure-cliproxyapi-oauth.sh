...
QWEN_IMPORT_SCRIPT="${QWEN_IMPORT_SCRIPT:-./scripts/cliproxyapi/import-qwen-auth.sh}"
...
show_usage() {
    cat <<'EOF_USAGE'
Usage: ./scripts/cliproxyapi/configure-cliproxyapi-oauth.sh [all|codex|qwen|kimi] [--help]

Examples:
  ./scripts/cliproxyapi/configure-cliproxyapi-oauth.sh
  ./scripts/cliproxyapi/configure-cliproxyapi-oauth.sh all
  ./scripts/cliproxyapi/configure-cliproxyapi-oauth.sh qwen codex

Behavior:
  - qwen: controlled by CLIPROXYAPI_QWEN_AUTH_MODE
      auto  -> import if source file exists, else native -qwen-login OAuth
      import -> imports ~/.qwen/oauth_creds.json using ./scripts/cliproxyapi/import-qwen-auth.sh
...
main() {
    ...
    if [ ! -f "$CLIPROXYAPI_CONFIG" ]; then
        ...
        print_error "Run ./scripts/cliproxyapi/start-cliproxyapi.sh once to generate config.yaml in oauth mode"
        return 1
    fi

...
CLIPROXYAPI_SETUP_SCRIPT="${CLIPROXYAPI_SETUP_SCRIPT:-./scripts/cliproxyapi/setup-cliproxyapi.sh}"
...
validate_provider_config() {
    ...
    if [ "$CLIPROXYAPI_PROVIDER_MODE" = "oauth" ]; then
        if [ ! -d "$CLIPROXYAPI_AUTH_DIR_PATH" ]; then
            ...
            print_info "Run ./scripts/cliproxyapi/configure-cliproxyapi-oauth.sh to populate credentials"
            return 1
        fi
        if ! find "$CLIPROXYAPI_AUTH_DIR_PATH" -maxdepth 2 -type f -name '*.json' | grep -q .; then
            ...
            print_info "Run ./scripts/cliproxyapi/configure-cliproxyapi-oauth.sh to create/update provider credentials"
            return 1
        fi
        return 0
    fi
...
ensure_binary() {
    ...
    if ! is_true "$CLIPROXYAPI_INSTALL_ON_START"; then
        ...
        print_info "Set CLIPROXYAPI_INSTALL_ON_START=true or run ./scripts/cliproxyapi/setup-cliproxyapi.sh"
        return 1
    fi
...
show_usage() {
    cat <<'EOF_USAGE'
Usage: ./scripts/cliproxyapi/start-cliproxyapi.sh [--help]
...
Environment variables:
  ...
  CLIPROXYAPI_SETUP_SCRIPT           Installer script path (default: ./scripts/cliproxyapi/setup-cliproxyapi.sh)

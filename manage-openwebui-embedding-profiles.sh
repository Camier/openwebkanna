...
show_help() {
    cat <<'EOF'
Usage: ./manage-openwebui-embedding-profiles.sh <command> [options]
...
cmd_use_kb() {
    ...
    if [ -n "$kb_key" ]; then
        ...
        if [ -z "$kb_id" ]; then
            ...
            print_info "  ./manage-openwebui-embedding-profiles.sh lanes"
            exit 1
        fi
    fi
    ...
    if [ -n "$explicit_profile_id" ]; then
        ...
        if [ -z "$resolved_profile_id" ]; then
            ...
            print_info "  ./manage-openwebui-embedding-profiles.sh bind-kb --kb-id $target_kb_id --profile <PROFILE_ID>"
            exit 1
        fi
    fi

...
- `VECTOR_DB=pgvector` stores vectors in PostgreSQL. In this repo the normal baseline is the Docker-managed `postgres` service, but a host or external PostgreSQL endpoint is also supported when `PGVECTOR_DB_URL` is set accordingly.

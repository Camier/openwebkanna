"""Fusion policy helpers for lane-based retrieval.

Weighted RRF configuration for text and visual retrieval lanes.
"""
...
def weights_for_query_type(
    query_type: str, policy: FusionPolicy | None = None
) -> LaneWeights:
    ...
    return LaneWeights(
        ...
        visual=policy.visual_lane_weight_text,
    )
    ...
    *,
    ...
    for lane_name, lane_hits, lane_weight in lane_specs:
        for rank, hit in enumerate(lane_hits, start=1):
            ...
            record["score"] = max(
                float(record.get("score", 0.0)), float(hit.get("score", 0.0))
            )
